...
def _get_default_node_parser(chunk_size_limit: Optional[int] = None,tokenizer: Optional[Callable[[str], List]] = None) -> NodeParser:
    """Get default node parser."""
    if chunk_size_limit is None:
        # use default chunk size (3900)
        token_text_splitter = TokenTextSplitter()
    else:
        token_text_splitter = TokenTextSplitter(tokenizer=tokenizer,chunk_size=chunk_size_limit)
...
class ServiceContext:
    ...
    def from_defaults(
        ...
        prompt_helper = prompt_helper or PromptHelper.from_llm_predictor(
            llm_predictor, chunk_size_limit=chunk_size_limit,tokenizer=tokenizer,
        )
        node_parser = node_parser or _get_default_node_parser(
            chunk_size_limit=chunk_size_limit,tokenizer=tokenizer
        )
